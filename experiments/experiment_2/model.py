...
def get_models():
    ...
    z_mean_dense = Dense(intermediate_dim, activation='relu')
    z_log_var_dense = Dense(intermediate_dim, activation='relu')
    ...
    hid_mean = z_mean_dense(hid)
    hid_var = z_log_var_dense(hid)

    z_mean_tensor = z_mean(hid_mean)
    z_log_var_tensor = z_log_var(hid_var)
